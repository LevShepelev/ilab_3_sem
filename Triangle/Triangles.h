...
class Triangle
    ...
    public: //getters
        ...
        char min_coord(int i) const {if (0 <= i && i <= 2) return p_[min_coord_[i]][i]; else return poison;}
        char max_coord(int i) const {if (0 <= i && i <= 2) return p_[max_coord_[i]][i]; else return poison;}
...
Triangle::Triangle(Vector p1, Vector p2, Vector p3, int n) : p_({p1, p2, p3}), plane_(p1, p2, p3), number_(n)
    ...
    for (int i = 0; i < 3; i++)// i is coordinate 0 = x, 1 = y, 2 = z;
        ...
        if (p_[1][i] > p_[2][i])
            { 
            if (p_[1][i] > p_[3][i]) max_coord_[i] = 1;
            else max_coord_[i] = 3;
            if (p_[2][i] < p_[3][3]) min_coord_[i] = 2;
            else min_coord_[i] = 3;
            }
        else 
            {
            if (p_[2][i] > p_[3][i]) max_coord_[i] = 2;
            else max_coord_[i] = 3;
            if (p_[1][i] < p_[3][i]) min_coord_[i] = 1;
            else min_coord_[i] = 3;
            }
        }
    line_[0] = Line(p1, p2);
...
void Calculating_the_task()
    ...
    for (int i = 1; i < N; i++)
        if (triangles[i] -> min_coord(0) <= triangles[i - 1] -> max_coord(0))
            if (triangles[i] -> min_coord(1) <= triangles[i - 1] -> max_coord(1))
                if (triangles[i] -> min_coord(2) <= triangles[i - 1] -> max_coord(2))
                    if (Triangle_crossing(*triangles[i], *triangles[i - 1]))
                        {
            ...
            for (int j = 1; j <= 3; j++)
                if (tr2.line(2).Is_valid())
                    if (Is_lines_in_one_plane(tr1.line(i), tr2.line(j)))
                        ...
                        if (p.Is_point_between_other_points(tr1.line(i).point(), tr1.line(i).direction() + tr1.line(i).point()) && p.Is_point_between_other_points(tr2.line(j).point(), tr2.line(j).direction() + tr2.line(j).point()))
                            return true;
                        }
    return false;
